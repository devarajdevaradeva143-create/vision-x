...
export default function Sidebar({ collapsed, onToggle }) {
  ...
  return (
    <aside className={`bg-[#0D2B43] text-gray-100 lg:fixed lg:inset-y-0 lg:left-0 lg:z-20 lg:flex lg:flex-col transition-[width] duration-200 ease-in-out ${widthClass}`}>
      <div className="flex items-center justify-between border-b border-blue-800 px-4 py-4 lg:px-5">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-[#0D2B43] text-amber-400">
            <Scale size={26} />
          </div>
          {!collapsed && (
      ...
      <div className="flex justify-end px-2 py-2 lg:justify-center">
        <button
          onClick={onToggle}
          className="inline-flex items-center justify-center rounded-md border border-blue-700 bg-[#0D2B43] p-2 text-blue-100 hover:bg-blue-700 hover:text-white"
          title={t('portalName')}
        >
        ...
        {collapsed ? (
          <div className="flex justify-center">
            <button
              ...
              className="flex items-center justify-center rounded-md border border-blue-700 bg-[#0D2B43] p-2 text-red-200 hover:bg-blue-700 hover:text-red-100"
              title={t('logout')}
            >
              <LogOut size={16} />
            </button>
          ...
          <>
            ...
            <button
              onClick={handleLogout}
              className="flex w-full items-center gap-2 rounded-md border border-blue-700 bg-[#0D2B43] px-3 py-2 text-sm font-semibold text-red-200 hover:bg-blue-700 hover:text-red-100"
            >
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        `flex shrink-0 items-center gap-3 rounded-md px-4 py-2.5 text-sm whitespace-nowrap lg:whitespace-normal lg:rounded-none lg:border-l-4 lg:px-5 lg:py-2.5 ${
          ...
          isActive
            ? 'border-amber-400 bg-[#0D2B43] font-semibold text-white'
            : 'border-transparent text-blue-100 hover:bg-[#0D2B43] hover:text-white'
        }`
      }
    >
