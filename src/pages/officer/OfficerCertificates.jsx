...
export default function OfficerCertificates() {
  ...
  return (
    <div>
      <PageHeader title="Issued Certificates" subtitle="Certificates you have issued after verification" />
      {myCerts.length === 0 ? (
        <EmptyState message="You have not issued any certificates yet." />
      ) : (
        <Card>
          <Table headers={['Certificate No.', 'Instrument', 'Owner', 'Issue Date', 'Expiry Date', 'Validity', '']}>
            {myCerts.map(cert => {
